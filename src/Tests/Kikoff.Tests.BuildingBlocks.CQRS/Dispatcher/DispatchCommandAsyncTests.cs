...
public class DispatchCommandAsyncTests
{
    ...
    public void Setup()
    {
        ...
        _serviceCollection.AddScoped<ICommandHandler<DummyCommand, Guid>, DummyHandler>();
    }
    ...
    public async Task ShouldExecuteRegisteredPipeline()
    {
        ...
        // Act
        var id = Guid.NewGuid();
        var command = new DummyCommand(id);
        Guid result = await dispatcher.DispatchCommandAsync<DummyCommand, Guid>(command);

        // Assert
        Assert.That(result, Is.EqualTo(id));
        Assert.That(registry.PipelineLog, Has.Count.EqualTo(3));
        Assert.That(registry.PipelineLog[0], Is.EqualTo(typeof(PipelineBehavior1<DummyCommand, Guid>)));
        Assert.That(registry.PipelineLog[1], Is.EqualTo(typeof(PipelineBehavior2<DummyCommand, Guid>)));
    ...
    internal record DummyCommand(Guid Id) : ICommand<Guid>;
    ...
    internal class DummyHandler : ICommandHandler<DummyCommand, Guid>
    {
        private readonly Registry _registry;
